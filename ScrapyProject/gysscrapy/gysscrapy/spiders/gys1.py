...
class GysSpider(scrapy.Spider):
    ...
    start_urls = ['https://shenzhen.china.cn/search/5146ns.shtml?t=1']

    def parse(self, res):
        lis = res.xpath("//div[@class='corplist']/div[@class='corpinfo']")
        for li in lis:
            # 获取公司名称
            name = li.xpath("h3").extract_first()
            name = re.sub("<.*?>", "", name) if name else ""
            # 获取url
            url = li.xpath("h3/a/@href").extract_first()
            # 获取主营产品
            product = li.xpath("./p/text()").extract_first()
